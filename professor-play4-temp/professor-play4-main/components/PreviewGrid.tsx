...
import { Download, Sparkles, ShoppingCart, ScanSearch } from 'lucide-react'
          ...
          return Array.from({ length: quantity }).map((_, i) => {
            ...
            return (
              <div key={`${groupIndex}-${i}`} className="flex flex-col gap-1.5 sm:gap-2 w-full max-w-[140px] sm:max-w-[150px]">
                ...
                >
                  {/* Hover Overlay for Inspector - Positioned on parent to stay visible during flip */}
                  <div className="absolute inset-0 pointer-events-none z-50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                    <div className="bg-white/90 text-slate-900 px-3 py-1.5 rounded-full text-[10px] font-bold shadow-lg flex items-center gap-1.5 transform scale-95 group-hover:scale-100 transition-transform">
                      <ScanSearch className="w-3.5 h-3.5" /> Inspect / Edit
                    </div>
                  </div>
                ...
                <div className="flex justify-between items-center px-1.5 sm:px-1 bg-slate-50 dark:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 py-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      const instanceIndex = i // Which copy within the group was clicked (0-indexed)

                      setDeck(prev => {
                        const newDeck = [...prev]
                        const group = { ...newDeck[groupIndex] }
                        const groupQty = group.quantity || 1

                        // If selecting Silver, we need to open inspector to configure it
                        if (newFinish.includes('silver')) {
                          if (groupQty > 1) {
                            // Split group based on which instance was clicked
                            const cardsBefore = instanceIndex // Cards before the clicked one
                            const cardsAfter = groupQty - instanceIndex - 1 // Cards after the clicked one

                            // Create the new card with the new finish
                            const newCard = {
                              ...group,
                            ...
                            if (cardsBefore > 0) {
                              // Keep cards before in original group
                              newDeck[groupIndex] = { ...group, quantity: cardsBefore }
                              insertOffset = 1
                            } else {
                              // No cards before, remove original group placeholder
                              newDeck.splice(groupIndex, 1)
                            }

                            // Insert the new single card
                            const newCardIndex = groupIndex + insertOffset
                            newDeck.splice(newCardIndex, 0, newCard)

                            if (cardsAfter > 0) {
                              // Add remaining cards after
                              const afterCard = {
                                ...group,
                                id: String(Date.now() + Math.random() + 1),
                                quantity: cardsAfter,
                              }
                              newDeck.splice(newCardIndex + 1, 0, afterCard)
                            }

                            // Open inspector for the new card
                            setTimeout(() => {
                              setInspectorIndex(newCardIndex)
                            }, 100)
                          } else {
                          ...
                          if (groupQty > 1) {
                            // Split group based on which instance was clicked
                            const cardsBefore = instanceIndex
                            const cardsAfter = groupQty - instanceIndex - 1
                  ...
                  >
                    ...
                    <option value="rainbow" disabled style={{ color: '#9ca3af' }}>Rainbow (Coming Soon)</option>
                    <option value="gloss" disabled style={{ color: '#9ca3af' }}>Gloss (Coming Soon)</option>
                    <option value="silver" disabled style={{ color: '#9ca3af' }}>Spot Silver (Coming Soon)</option>
                    <option value="silver-rainbow" disabled style={{ color: '#9ca3af' }}>Silver+Holo (Coming Soon)</option>
                    <option value="silver-gloss" disabled style={{ color: '#9ca3af' }}>Silver+Gloss (Coming Soon)</option>
                  </select>
                </div>
              </div>
            )
